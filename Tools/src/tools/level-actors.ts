...
export function registerLevelActorTools(server: McpServer): void {
  server.tool(
    "attach_actor",
    "Attach a child actor to a parent actor in the current level. Requires editor mode. The child actor's transform is kept in world space by default. Optionally attach to a specific socket.",
    {
      ...
      socketName: z.string().optional().describe("Optional socket name on the parent to attach to"),
    ...
    async ({ childActor, parentActor, socketName, attachmentRule }) => {
      ...
      if (data.socketName) {
        lines.push(`Socket: ${data.socketName}`);
      }
      lines.push(`\nNext steps:`);
  ...
  server.tool(
    "detach_actor",
    "Detach an actor from its parent in the current level. The actor's world transform is preserved by default. Requires editor mode.",
    {
      actorLabel: z.string().describe("Label of the actor to detach from its parent"),
    ...
    async ({ actorLabel, detachmentRule }) => {
      ...
      const lines = [
        `Detached '${data.actorLabel}' from '${data.previousParent}'`,
      ];
      lines.push(`\nNext steps:`);
      lines.push(`  1. Use list_actors to verify the actor is now a root actor`);
      lines.push(`  2. Use set_actor_transform to reposition if needed`);
  ...
  server.tool(
    "duplicate_actor",
    "Duplicate an actor in the current level. The new actor is placed at the same location by default, or offset by the specified amount. Requires editor mode.",
    {
      ...
      offset: z.object({
        x: z.number().optional().describe("X offset from source (default: 0)"),
        y: z.number().optional().describe("Y offset from source (default: 0)"),
        z: z.number().optional().describe("Z offset from source (default: 0)"),
    ...
    async ({ actorLabel, newLabel, offset }) => {
      ...
      lines.push(`\nNext steps:`);
      lines.push(`  1. Use set_actor_transform to reposition the duplicate`);
      lines.push(`  2. Use set_actor_property to modify the duplicate's properties`);
      lines.push(`  3. Use rename_actor to give it a meaningful name`);
  ...
  server.tool(
    "rename_actor",
    "Rename an actor's label in the current level. This changes the display name shown in the World Outliner. Requires editor mode.",
    {
      actorLabel: z.string().describe("Current label of the actor to rename"),
    ...
    async ({ actorLabel, newLabel }) => {
      ...
      const lines = [
        `Renamed '${data.oldLabel}' to '${data.newLabel}'`,
        `Class: ${data.actorClass}`,
      ];
      lines.push(`\nNext steps:`);
      lines.push(`  1. Use list_actors to verify the renamed actor`);
