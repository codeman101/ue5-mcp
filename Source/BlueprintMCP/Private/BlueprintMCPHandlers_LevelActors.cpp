...
// ============================================================
// HandleAttachActor — attach one actor to another
// ============================================================
...
FString FBlueprintMCPServer::HandleAttachActor(const FString& Body)
{
	...
	UE_LOG(LogTemp, Display, TEXT("BlueprintMCP: attach_actor(child='%s', parent='%s', socket='%s')"),
		*ChildLabel, *ParentLabel, *SocketName);

	if (!bIsEditor)
	{
		return MakeErrorJson(TEXT("attach_actor requires editor mode."));
	}
...
// ============================================================
// HandleDetachActor — detach an actor from its parent
// ============================================================
...
FString FBlueprintMCPServer::HandleDetachActor(const FString& Body)
{
	...
	if (!bIsEditor)
	{
		return MakeErrorJson(TEXT("detach_actor requires editor mode."));
	}
...
// ============================================================
// HandleDuplicateActor — duplicate an actor in the level
// ============================================================
...
FString FBlueprintMCPServer::HandleDuplicateActor(const FString& Body)
{
	...
	if (!bIsEditor)
	{
		return MakeErrorJson(TEXT("duplicate_actor requires editor mode."));
	}
	...
	// Use editor selection + duplication workflow
	GEditor->SelectNone(false, true, false);
	GEditor->SelectActor(SourceActor, true, true);

	// Duplicate via editor command
	GEditor->edactDuplicateSelected(World->GetCurrentLevel(), false);

	// The duplicated actor should now be the selected actor
	...
	if (!NewActor)
	{
		return MakeErrorJson(TEXT("Duplication failed \u2014 no new actor was created."));
	}

	// Apply offset if specified
	if (OffsetX != 0 || OffsetY != 0 || OffsetZ != 0)
	{
		FVector NewLocation = NewActor->GetActorLocation() + FVector(OffsetX, OffsetY, OffsetZ);
		NewActor->SetActorLocation(NewLocation);
	}

	// Optionally apply a new label
	FString NewLabel;
...
// ============================================================
// HandleRenameActor — rename an actor's label
// ============================================================
...
FString FBlueprintMCPServer::HandleRenameActor(const FString& Body)
{
	...
	if (!bIsEditor)
	{
		return MakeErrorJson(TEXT("rename_actor requires editor mode."));
	}
